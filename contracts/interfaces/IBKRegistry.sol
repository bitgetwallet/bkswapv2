...
interface IBKRegistry {    
    ...
    function setCallTarget(bytes4 _methodId, address[] calldata _targets, bool _isEnable) external;

    function isCallTarget(bytes4 _methodId, address _target) external view returns(bool);

    function setApproveTarget(bytes4 _methodId, address[] calldata _targets, bool _isEnable) external;
